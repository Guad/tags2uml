...
func parseMembersMethods(fn string) {
	...
	ret := regexp.MustCompile(`\/\^([\s]*)([A-Za-z0-9_\.]+)([^A-Za-z0-9_]+)(.*)\$\/`)
	for scanner.Scan() {
		// log.Println("Scanning", scanner.Text())
		txt := scanner.Text()
		...
		matchc := rec.FindStringSubmatch(txt)
		ci := classinfo_st{}
		var cn string
		if len(matchc) != 0 {
			cn = matchc[1]
		...
		matcha := rea.FindStringSubmatch(txt)
		matchl := rel.FindStringSubmatch(txt)
		access1 := " "
		if len(matcha) != 0 {
			if matcha[1] == "public" {
				access1 = "+"
		...
		thetype := match[4]
		matcht := ret.FindStringSubmatch(remove_keywords(txt))
		datatype := ""
